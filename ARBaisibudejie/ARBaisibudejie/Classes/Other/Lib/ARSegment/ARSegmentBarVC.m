...
@interface ARSegmentBarVC ()<ARSegmentBarDelegate, UIScrollViewDelegate>//代理的使用 1
@property (nonatomic, weak) UIScrollView *contentV;
@end

@implementation ARSegmentBarVC
{
...
#pragma mark - 以下为懒加载
- (ARSegmentBar *)segmentBar {
    if (!_segmentBar) {
        ARSegmentBar *segmentBar = [ARSegmentBar segmentBarWithFrame:CGRectZero config:nil parentView:self.view];
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    //ARLog(@"scrollViewWillBeginDragging %f",self.contentV.contentOffset.x);
    _beginDraggingContentOffsetX = self.contentV.contentOffset.x;
}

-(void) scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset{
    //ARLog(@"scrollViewWillEndDragging %f",self.contentV.contentOffset.x);
