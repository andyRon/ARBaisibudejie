...
@interface ARSegmentBar()<UICollectionViewDelegate>
{
    ARSegmentConfig * _segmentConfig;
}
@property (nonatomic, strong) UIScrollView *contentScrollV;
@property (nonatomic, strong) UIView *indicatorV;
@property (nonatomic, strong) ARSegmentMoreBtn *showMoreBtn;
...
@property (nonatomic, strong) ARShowMoreVC *showMoreVC;

@end

@implementation ARSegmentBar

#pragma mark - 以下为懒加载

- (ARSegmentConfig *) segmentConfig{
    if (!_segmentConfig){
        _segmentConfig = [ARSegmentConfig defaultConfig];
    }
    return _segmentConfig;
}

- (ARShowMoreVC *)showMoreVC {
    if (!_showMoreVC) {
        _showMoreVC = [[ARShowMoreVC alloc] initWithConfig:self.segmentConfig];
...
- (ARSegmentMoreBtn *)showMoreBtn{
    if (!_showMoreBtn) {
        ...
        _showMoreBtn = [[ARSegmentMoreBtn alloc] init];
...
+ (instancetype)segmentBarWithFrame: (CGRect)frame config: (ARSegmentConfig *)config parentView: (UIView *)parentV {
    ...
    ARSegmentBar *segBar = [[ARSegmentBar alloc] initWithFrame:defaultFrame];
    if (config){
...
-(instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        //self.segmentConfig = [ARSegmentConfig defaultConfig];//移至懒加载
