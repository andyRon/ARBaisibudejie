...
typedef NS_ENUM(NSInteger, ARRefreshState) {
    RefreshStateNormal,
    RefreshStatePulling,
    RefreshStateRefresh,
};

@interface ARRefreshOC()
...
@property (assign,nonatomic) ARRefreshState refreshState;
...
+(instancetype) refreshWithTarget:(id)target action:(SEL)action config:(void(^)(ARRefreshOC * refresh)) block{
    ARRefreshOC * refresh = [[self alloc] initWithConfig:block];
    ...
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        switch (_kind) {
            ...
            case RefreshKindNormal:{
                ...
                (self.verticalAlignment == ARRefreshAlignmentTop ? (height/kBeginHeight) : 1);
...
-(void) setRefreshState:(ARRefreshState)refreshState{
...
-(void) normal :(ARRefreshState)refreshState {
...
-(UIImage *)getImage:(NSString *)name{
    NSBundle *currentBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[ARRefreshOC class]] pathForResource:@"ARRefreshOC" ofType:@"bundle"]];
