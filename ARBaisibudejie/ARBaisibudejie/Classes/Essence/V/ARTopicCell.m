...
@property (nonatomic, weak) ARPicV *picV;
@property (nonatomic, weak) ARVoiceV *voiceV;
@property (nonatomic, weak) ARVideoV *videoV;
...
static ARCommentM *lastCommentM_;


@implementation ARTopicCell

- (ARPicV *)picV{
    if (!_picV) {
        ARPicV *picV = [ARPicV viewFromXIB];
...
- (ARVoiceV *)voiceV{
    if (!_voiceV) {
        ARVoiceV *voiceV = [ARVoiceV viewFromXIB];
...
- (ARVideoV *)videoV{
    if (!_videoV) {
        ARVideoV *videoV = [ARVideoV viewFromXIB];
...
- (void)setTopic:(ARTopicM *)topic{
    ...
    if (topic.top_cmt){
        ...
        [attrStr addAttribute:NSForegroundColorAttributeName
                        value:ARColor(62, 114, 166)
                        range:NSMakeRange(0, username.length)];
        self.topCmtLbl.attributedText = attrStr;
    }
    
    self.picV.hidden = !(topic.type == ARTopicTypePicture);
    self.voiceV.hidden = !(topic.type == ARTopicTypeVoice);
    self.videoV.hidden = !(topic.type == ARTopicTypeVideo);
    if (topic.type == ARTopicTypePicture) { // 图片
        //        self.picV.hidden = NO;
        //        self.voiceV.hidden = YES;
        //        self.videoV.hidden = YES;
        self.picV.topic = topic;
    } else if (topic.type == ARTopicTypeVoice) { // 声音
        //        self.picV.hidden = YES;
        //        self.voiceV.hidden = NO;
        //        self.videoV.hidden = YES;
        self.voiceV.topic = topic;
    } else if (topic.type == ARTopicTypeVideo) { // 视频
        //        self.picV.hidden = YES;
        //        self.voiceV.hidden = YES;
        //        self.videoV.hidden = NO;
        self.videoV.topic = topic;
    } else if (topic.type == ARTopicTypeWord) { // 段子
        //        self.picV.hidden = YES;
        //        self.voiceV.hidden = YES;
        //        self.videoV.hidden = YES;
    }
...
//    self.leftConstraint.constant = self.topic.commentBtnX + Margin;
    ...
    if (self.topic.type == ARTopicTypePicture) { // 图片
        self.picV.frame = self.topic.middleFrame;
    } else if (self.topic.type == ARTopicTypeVoice) { // 声音
        self.voiceV.frame = self.topic.middleFrame;
    } else if (self.topic.type == ARTopicTypeVideo) { // 视频
