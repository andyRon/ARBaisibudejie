...
-(void) seeBigPic{
    ARSeeBigPicVC *vc = [[ARSeeBigPicVC alloc] init];
    vc.topic = self.topic;
...
- (void)setTopic:(ARTopicM *)topic{
    ...
    [self.imageV tg_setOriginImage:topic.image1 thumbnailImage:topic.image0 placeholder:nil progress:^(NSInteger receivedSize, NSInteger expectedSize,NSURL * _Nullable targetURL) {
    ...
    self.gifV.hidden = !topic.is_gif;
    self.gifV.hidden = ![topic.image1.lowercaseString hasSuffix:@"gif"];
    self.gifV.hidden = ![topic.image1.pathExtension.lowercaseString isEqualToString:@"gif"];
