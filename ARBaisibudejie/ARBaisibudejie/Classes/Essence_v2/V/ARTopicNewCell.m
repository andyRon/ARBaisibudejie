...
#import "ARUserNewM.h"
#import "ARPicNewV.h"
#import "ARVideoNewV.h"
#import "ARVoiceNewV.h"
#import "ARTopCommentCell.h"
#import <UIImageView+WebCache.h>
#import <AVFoundation/AVFoundation.h>
#import <MediaPlayer/MediaPlayer.h>

@interface ARTopicNewCell()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic, weak) ARPicNewV *picV;
@property (nonatomic, weak) ARVideoNewV *videoV;
@property (nonatomic, weak) ARVoiceNewV *voiceV;
@property (strong, nonatomic) AVPlayerItem *playerItem;
@property (nonatomic, strong) UIButton *coverBtn;

@end

static NSString *const commentID = @"ARTopCommentCellID";

@implementation ARTopicNewCell
...
- (ARVoiceNewV *)voiceV{
    if (!_voiceV) {
        ARVoiceNewV *voiceV = [ARVoiceNewV viewFromXIB];
...
- (ARPicNewV *)picV{
    if (!_picV) {
        ARPicNewV *picV = [ARPicNewV viewFromXIB];
...
- (ARVideoNewV *)videoV{
    if (!_videoV) {
        ARVideoNewV *videoV = [ARVideoNewV viewFromXIB];
...
- (void)awakeFromNib {
    ...
    [self.topCmtTableV registerNib:[UINib nibWithNibName:NSStringFromClass([ARTopCommentCell class]) bundle:nil] forCellReuseIdentifier:commentID];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ARTopCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:commentID forIndexPath:indexPath];
    cell.commentM = self.topic.top_comments[indexPath.row];
    return cell;
}

- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    //ARLog(@"%f",self.topic.top_comments[indexPath.row].topCommentCellHeight)
...
- (void)setTopic:(ARTopicNewM *)topic{
    ...
    [self.nameLbl setTextColor:topic.u.is_vip ? ARColor(240, 77, 71) : ARColor(0, 128, 128) ];
