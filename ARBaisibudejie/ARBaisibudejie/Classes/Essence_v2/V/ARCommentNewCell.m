...
static ARCommentNewM *lastCommentM_;
static NSTimer *avTimer_;
static UIProgressView *progress_;

@implementation ARCommentNewCell
...
-(void)setCommentM:(ARCommentNewM *)commentM{
    _commentM = commentM;
    ...
    if (commentM.user.total_cmt_like_count >= 1000) {
        ...
        self.totalLikeCountLbl.backgroundColor = commentM.user.total_cmt_like_count >= 10000 ? ARColor(250, 195, 198) : ARColor(212, 205, 214);
    } else {
        self.totalLikeCountLbl.text = [NSString stringWithFormat:@"%zd", commentM.user.total_cmt_like_count];
        self.totalLikeCountLbl.backgroundColor = ARColor(191, 205, 224);
    }
...
- (void)timer{
    ...
    if (currentTime > 0){
        ...
        ARLog(@" --- progress %f --- ",progress_.progress)
    }
}
...
- (IBAction)videoPlay:(UIButton *)playBtn {
    ...
    //ARLog(@"%zd %@ %@",progress_.hidden,NSStringFromCGRect(playerLayer_.frame),NSStringFromCGRect(progress_.frame));
}
