...
+ (NSDictionary *)mj_objectClassInArray {
    return @{
             @"top_comments" : [ARCommentNewM class]
             };
}
...
- (CGFloat)cellHeight{
    ...
    if (![self.type isEqualToString: @"text"]) { //图片、视频
        ...
        //ARLog(@"%ld,%ld, %@ , %@ ,%@", self.height, self.width,self.type,self.text, NSStringFromCGRect(_middleFrame))
    ...
    if (self.top_comments.count > 0){//评论
        ...
        for (int i=0 ; i <self.top_comments.count; i++){
            ARCommentNewM * com = self.top_comments[i];
    ...
    //ARLog(@"_cellHeight,%@,%@,%f",self.type, self.text, _cellHeight)
