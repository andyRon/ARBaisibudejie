...
#import "ARRefreshOC.h"


@interface ARLookingAroundVC ()<UICollectionViewDataSource,UICollectionViewDelegate, ARLayoutDelegate>
...
@property (nonatomic ,weak) ARRefreshOC *refresh;

@end

static NSString * const ARCollectionViewCellId = @"LookingAroundCellId";

@implementation ARLookingAroundVC
{
...
- (void)viewDidLoad {
    ...
    _np = @"0";
    self.view.backgroundColor = ARGrayColor(246);
...
- (void)setupRefresh{
    ...
    //换用自己的刷新控件
    ARRefreshOC *refresh = [ARRefreshOC new];
    //    refresh.kind = RefreshKindNormal;
    //refresh.bgColor =  [UIColor colorWithWhite:0.8 alpha:1];
    refresh.verticalAlignment = ARRefreshAlignmentMidden;
...
- (void)setupLayout{
    ARLayout *layout = [[ARLayout alloc] init];
    layout.delegate = self;
    
    UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:self.view.bounds collectionViewLayout:layout];
    collectionView.backgroundColor = ARGrayColor(246);
    collectionView.dataSource = self;
    collectionView.delegate = self;
    [self.view addSubview:collectionView];
    
    [collectionView registerNib:[UINib nibWithNibName:NSStringFromClass([ARLookingAroundV class]) bundle:nil] forCellWithReuseIdentifier:ARCollectionViewCellId];
...
-(void)getNextLikeIndexWithTopicId :(NSString *)topicId :(void(^)(NSInteger nextLikeIndex))block{
    ...
    [weakSelf.topics enumerateObjectsUsingBlock:^(ARTopicNewM * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ARLookingAroundV *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ARCollectionViewCellId forIndexPath:indexPath];
    ...
    [cell setNextBlock:^(NSString * topicId){//循环播放喜欢，喜欢的在界面则动画切到喜欢的cell上，若不在，则直接替换playerItem
        ...
        [self getNextLikeIndexWithTopicId:topicId :^(NSInteger nextLikeIndex) {
            if (nextLikeIndex > -1){
                ...
                //在界面上
                for (ARLookingAroundV * obj in arr2) {
...
-(void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath{
    ARLookingAroundV * voiceCell =  (ARLookingAroundV *)cell;
    if (!voiceCell.topic.isAnimated){
        //ARLog(@"frame %@",NSStringFromCGRect(cell.frame))
        //效果1
