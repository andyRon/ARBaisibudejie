...
@interface ARSeeBigPicNewVC ()<UIScrollViewDelegate>
...
@implementation ARSeeBigPicNewVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    //    ARLog(@"viewDidLoad %@",NSStringFromCGRect(self.view.bounds))//{{0, 0}, {600, 600}}
    ...
    //ARLog(@"%@,%@",NSStringFromCGRect(imageView.frame),self.topic.image)
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [_imageV tg_setOriginImage:self.topic.image thumbnailImage:self.topic.image_small placeholder:nil progress:^(NSInteger receivedSize, NSInteger expectedSize,NSURL * _Nullable targetURL) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //ARLog(@"实际宽高 %f,%f | 模型宽高 %ld,%ld | 图片控件宽高%f,%f",w,h,self.topic.width,self.topic.height,_imageV.width,_imageV.height);
    ...
    //    ARLog(@"%zd",scrollView.autoresizingMask)//自己代码加的为0 ，如果是XIB加的为18
}

//-(void) viewDidAppear:(BOOL)animated{
//    ARLog(@"viewDidAppear %@",NSStringFromCGRect(self.view.bounds))//{{0, 0}, {414, 736}}
//}
